 ...
 */
export async function runAfter(projectRoot = process.cwd(), deps) {
    ...
    // ── DECISION ─────────────────────────────────────────────────────
    const assumption = (await ask('What assumption broke or needed correction? (press enter to skip)')).trim();
    if (assumption) {
        const correction = (await ask('What was the correction? What is the right understanding?')).trim();
        const component = (await ask('Which files or components does this apply to? (comma-separated paths preferred)')).trim();
    ...
    // ── DANGER ZONE ──────────────────────────────────────────────────
    const dangerPattern = (await ask('Record a danger zone for future sessions? Describe the pattern to watch for, or press enter to skip.')).trim();
    if (dangerPattern) {
        const component = (await ask('Which files or components does this danger apply to? (comma-separated paths preferred)')).trim();
