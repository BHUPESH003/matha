 ...
 */
export async function runMigrate(projectRoot = process.cwd(), deps) {
    ...
    if (current && current > CURRENT_SCHEMA_VERSION) {
        const message = `This .matha/ uses schema v${current}, newer than this MATHA (v${CURRENT_SCHEMA_VERSION}). ` +
            'Upgrade MATHA: npm install -g @10kdevs/matha';
        log(message);
        return { exitCode: 1, message };
    }
    await writeAtomic(configPath, { ...config, schema_version: CURRENT_SCHEMA_VERSION }, { overwrite: true });
