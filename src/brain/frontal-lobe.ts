import type { DangerZone, DecisionEntry } from '@/brain/hippocampus.js'
...
export interface SessionContext {
  ...
  operationType:
    ...
    | 'architecture'
    | 'frozen_component'
  stabilityData?: Record<string, string>
}
...
export interface SessionBrief {
  ...
  contract: string[]
  modelTier: 'lightweight' | 'mid' | 'capable'
  tokenBudget: number
...
interface BriefHippocampus {
  getDangerZones: (context?: string) => Promise<DangerZone[]>
}

interface WriteBackHippocampus {
  mathaDir: string
  recordDecision: (mathaDir: string, entry: DecisionEntry) => Promise<void>
  recordDangerZone: (
    mathaDir: string,
    zone: DangerZone,
  ) => Promise<void>
...
export interface WriteBackDiscovery {
...
export function runGate(
  ...
  try {
    switch (gateId) {
      ...
      case 3: {
        const orient =
          isNonEmptyObject(input) || isNonEmptyObject(context.stabilityData)
        return {
          gateId,
          completed: orient,
      ...
      case 7: {
        const states = Array.isArray(input) ? (input as GateState[]) : []
 ...
 */
export function validateSequence(states: GateState[]): SequenceValidation {
...
export async function generateBrief(
  context: SessionContext,
  states: GateState[],
  hippocampus: BriefHippocampus,
): Promise<SessionBrief> {
  const stateMap = new Map<number, GateState>()
  for (const s of states) stateMap.set(s.gateId, s)
  ...
  const readyFromGate7 =
    typeof gate7?.output === 'object' &&
    gate7?.output !== null &&
    'readyToBuild' in gate7.output
      ? Boolean((gate7.output as { readyToBuild?: boolean }).readyToBuild)
      : undefined

  const readyToBuild =
    readyFromGate7 ?? findMissingRequiredBuildGates(states).length === 0
...
export async function runWriteBack(
  context: SessionContext,
  discovery: WriteBackDiscovery,
  hippocampus: WriteBackHippocampus,
): Promise<void> {
  const correction = discovery.correction?.trim() ?? ''
  const dangerPattern = discovery.dangerPattern?.trim() ?? ''

  if (!correction && !dangerPattern) return
  ...
  if (correction) {
    ...
    const decision: DecisionEntry = {
      ...
      previous_assumption:
        discovery.previousAssumption?.trim() || 'No previous assumption recorded',
      correction,
  ...
  if (dangerPattern) {
    const zone: DangerZone = {
      ...
      description:
        discovery.dangerDescription?.trim() ||
...
function findMissingRequiredBuildGates(states: GateState[]): number[] {
  const required = [1, 2, 3, 4, 5]
  const missing: number[] = []

  for (const gateId of required) {
    const state = states.find((s) => s.gateId === gateId)
    if (!state || !state.completed) missing.push(gateId)
  }
...
): { modelTier: 'lightweight' | 'mid' | 'capable'; tokenBudget: number } {
