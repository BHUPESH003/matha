...
export interface DeltaRecord {
  ...
  contract_result: 'passed' | 'partial' | 'violated' | 'none';
...
export interface RoutingRule {
  ...
  recommended_tier: 'lightweight' | 'mid' | 'capable';
...
export interface DopamineAnalysis {
  ...
  globalAvgTokenDelta: number;
  overBudgetRate: number;
}

const DEFAULT_TIERS: Record<string, { tier: 'lightweight' | 'mid' | 'capable', budget: number }> = {
...
export async function analyseDeltas(mathaDir: string): Promise<DopamineAnalysis> {
  const emptyAnalysis: DopamineAnalysis = {
  ...
  try {
    ...
    const deltas = await readJsonOrNull<DeltaRecord[]>(deltasPath);
    ...
    if (validDeltas.length > 0) {
      const sum = validDeltas.reduce((acc, d) => acc + (d.token_delta as number), 0);
      globalAvgTokenDelta = sum / validDeltas.length;
      
      const overBudgetCount = validDeltas.filter(d => (d.token_delta as number) > 0).length;
...
export function buildRoutingRules(deltas: DeltaRecord[]): RoutingRule[] {
  const groups: Record<string, DeltaRecord[]> = {};
  ...
  for (const [opType, records] of Object.entries(groups)) {
    ...
    const sum = validRecords.reduce((acc, r) => acc + (r.token_delta as number), 0);
    ...
    if (avgTokenDelta > 5000) {
      if (recommendedTier === 'lightweight') recommendedTier = 'mid';
      else if (recommendedTier === 'mid') recommendedTier = 'capable';
    } else if (avgTokenDelta < -2000) {
      if (recommendedTier === 'capable') recommendedTier = 'mid';
      else if (recommendedTier === 'mid') recommendedTier = 'lightweight';
    }
...
export function buildConfidence(deltas: DeltaRecord[]): ComponentConfidence[] {
  const groups: Record<string, DeltaRecord[]> = {};
  for (const d of deltas) {
  ...
  const confidences: ComponentConfidence[] = [];
  ...
  for (const [tier, records] of Object.entries(groups)) {
    ...
    if (validTokens.length > 0) {
      avgTokenDelta = validTokens.reduce((s, r) => s + (r.token_delta as number), 0) / validTokens.length;
    }
    ...
    for (const r of records) {
      ...
      if (r.contract_result === 'passed') adjustment += 0.1;
      else if (r.contract_result === 'violated') adjustment -= 0.2;
      else if (r.contract_result === 'partial') adjustment -= 0.1;

      if (r.token_delta !== null) {
        if (r.token_delta > 5000) adjustment -= 0.1;
        else if (r.token_delta < 0) adjustment += 0.05;
      }
    }
...
export async function getRecommendation(mathaDir: string, operationType: string): Promise<{ 
  ...
  try {
    ...
    const analysis = await readJsonOrNull<DopamineAnalysis>(rulesPath);
...
export async function persistAnalysis(mathaDir: string, analysis: DopamineAnalysis): Promise<void> {
  try {
    const rulesPath = path.join(mathaDir, 'dopamine/routing-rules.json');
