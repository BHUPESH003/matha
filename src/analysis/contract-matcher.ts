...
export function matchDangerZones(context: MatchContext, dangerZones: any[]): MatchResult[] {
...
export function matchContracts(context: MatchContext, contracts: Record<string, any>): MatchResult[] {
  const results: MatchResult[] = []
  const scopeLower = context.scope.toLowerCase()
  ...
  for (const [key, contract] of Object.entries(contracts)) {
    ...
    if (scopeLower.includes(compLower)) {
      const violations = (contract.assertions || []).filter((a: any) => a.violation_count > 0)
...
export function matchFrozenFiles(context: MatchContext, stabilityRecords: any[]): MatchResult[] {
  const results: MatchResult[] = []
...
export function matchDecisionPatterns(context: MatchContext, decisions: any[]): MatchResult[] {
  const results: MatchResult[] = []
  const scopeLower = context.scope.toLowerCase()
...
const SEVERITY_ORDER: Record<MatchSeverity, number> = {
  critical: 0,
  warning: 1,
  info: 2,
}

export async function matchAll(context: MatchContext, mathaDir: string): Promise<MatchResult[]> {
  try {
