...
export async function writeAtomic(
  ...
  try {
    ...
    throw new Error(
      `Data is not JSON-serializable: ${(err as Error).message}`,
    )
  }
  ...
  if (!options?.overwrite) {
    ...
    if (exists) {
      throw new Error(
        `File already exists and overwrite is not enabled: ${filePath}`,
      )
    }
  }
...
export async function appendToArray(
  ...
  try {
    ...
    if (!Array.isArray(parsed)) {
      throw new Error(
        `Cannot append to non-array. File contains ${typeof parsed}`,
      )
    }
    existing = parsed
  } catch (err: any) {
...
export async function mergeObject(
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        `Cannot merge into non-object. File contains ${Array.isArray(parsed) ? 'array' : typeof parsed}`,
      )
    }
