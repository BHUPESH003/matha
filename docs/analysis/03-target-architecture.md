...
- **Next decision point:** field validation of v1.0.0 on live large codebases (owner-driven), then iterate from findings. Deferred beyond v1.0: Stop-hook write-back prompt, monorepo directory rollups, interactive catchup backfill, danger-zone timestamps (possiblyStale for zones).
